...
class TaskRunner(BaseTaskRunner):
    ...
    def populate_cache(self):
        ...
        for definition in definitions:
            ...
            #write_check_log(f"Populate_cache{definition.job_definition_id} => next_run_time={next_run_time} => schedule={definition.schedule} ")
            self.cache.put(
    ...
    def add_job_definition(self, job_definition_id: str):
        ...
        if definition.active:
            #write_check_log(f"Push Queue {definition.job_definition_id} => next_run_time={next_run_time} => schedule={definition.schedule} ")
            self.queue.push(
                JobDefinitionTask(
    ...
    def create_job(self, job_definition_id: str):
        ...
        if definition and definition.active:
            ...
            self.scheduler.create_job(
                CreateJob(
                    **definition.dict(exclude={"schedule", "timezone", "input_uri"}, exclude_none=True),
                    input_uri=input_uri,
                )
            )
    ...
    def process_queue(self):
        ...
        while not self.queue.isempty():
            ...
            #write_check_log(f"process_queue {task.job_definition_id} => time_diff={time_diff} => schedule={task.job_definition_id} ")
